...
namespace Soenneker.Persona.Runners.OpenApiClient.Utils;

///<inheritdoc cref="IFileOperationsUtil"/>
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    private const string _personaOpenApiRepositoryUrl = "https://github.com/persona-id/persona-openapi";
    private const string _personaOpenApiDefaultDocumentName = "openapi-bundled.json";
    private static readonly Regex _datedFolderRegex = new(@"^\d{4}-\d{2}-\d{2}$", RegexOptions.Compiled);
    ...
    public FileOperationsUtil(ILogger<FileOperationsUtil> logger, IConfiguration configuration, IGitUtil gitUtil, IDotnetUtil dotnetUtil,
        IProcessUtil processUtil, IFileDownloadUtil fileDownloadUtil, IFileUtil fileUtil, IDirectoryUtil directoryUtil, IOpenApiFixer openApiFixer, IKiotaUtil kiotaUtil)
    {
        _logger = logger;
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        await _kiotaUtil.Generate(fixedFilePath, "PersonaOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
    ...
    {
        string? configuredOpenApiDocumentUrl = _configuration["Persona:ClientGenerationUrl"];

        if (!string.IsNullOrWhiteSpace(configuredOpenApiDocumentUrl))
        {
            _logger.LogInformation("Using configured Persona OpenAPI document URL: {OpenApiDocumentUrl}", configuredOpenApiDocumentUrl);
            return configuredOpenApiDocumentUrl;
        }

        string latestFolder = await GetLatestOpenApiFolder(cancellationToken);
        string openApiDocumentUrl = $"https://raw.githubusercontent.com/persona-id/persona-openapi/main/{latestFolder}/{_personaOpenApiDefaultDocumentName}";

        _logger.LogInformation("Resolved latest Persona OpenAPI document URL: {OpenApiDocumentUrl}", openApiDocumentUrl);

        return openApiDocumentUrl;
    }

    private async ValueTask<string> GetLatestOpenApiFolder(CancellationToken cancellationToken)
    {
